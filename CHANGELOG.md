...
- 좋아요/공유 버튼 디자인 개선
- 좋아요를 회원 전용 기능으로 변경
- 문서 저장 버튼과 회원 전용 저장 카운트 추가
- 설치 화면에서 사이트 이름과 파비콘을 설정할 수 있도록 추가
- 관리자 기본 설정에서 사이트 이름과 파비콘 수정 기능 추가
- 관리자 이미지 관리 탭과 WebP 변환 저장 기능 추가
- 관리자 기본 설정에서 프론트 `<head>` 추가 코드 관리 기능 추가

### Fixed

- 일부 MySQL/NAS 환경에서 `note_likes` 외래키 생성 실패 또는 테이블 미생성으로 문서 상세 페이지가 500 오류를 내던 문제 수정
- 루트 도메인 배포 환경에서 공유 URL이 하위 경로를 포함하던 문제 수정
